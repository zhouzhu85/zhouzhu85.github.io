...
2. 从库读取主控的二进制日志文件binlog，写入到从库的中继日志relay log
3. 从库再启动线程执行relay log日志，同步数据到从库 

## 分库分表
### 分库分表的时机：
1. 前提，项目业务数据逐渐增多，或业务发展比较迅速，单表的数据量达到1000w或20G以后，就要分库分表
2. 优化已经解决不了性能问题，比如主从读写分离，查询索引等解决不了，就要分库分表
3. io瓶颈（磁盘io、网咯io）、cup瓶颈（聚合查询、连接数太多）

### 拆分策略
![img.png](../images/img-db.png)
...
![img.png](../images/img-db2.png)

分库分表问题
![img_1.png](../images/img-db3.png)
